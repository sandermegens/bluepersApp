...
public class WedstrijdResourceIntTest {
    ...
    @Mock
    private WedstrijdRepository wedstrijdRepositoryMock;

    @Autowired
    private WedstrijdMapper wedstrijdMapper;
    
    @Mock
    private WedstrijdService wedstrijdServiceMock;
    ...
    public void getAllWedstrijdsWithEagerRelationshipsIsEnabled() throws Exception {
        WedstrijdResource wedstrijdResource = new WedstrijdResource(wedstrijdServiceMock);
        when(wedstrijdServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        MockMvc restWedstrijdMockMvc = MockMvcBuilders.standaloneSetup(wedstrijdResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
            .setControllerAdvice(exceptionTranslator)
            .setConversionService(createFormattingConversionService())
            .setMessageConverters(jacksonMessageConverter).build();

        restWedstrijdMockMvc.perform(get("/api/wedstrijds?eagerload=true"))
        .andExpect(status().isOk());

        verify(wedstrijdServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    public void getAllWedstrijdsWithEagerRelationshipsIsNotEnabled() throws Exception {
        WedstrijdResource wedstrijdResource = new WedstrijdResource(wedstrijdServiceMock);
            when(wedstrijdServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
            MockMvc restWedstrijdMockMvc = MockMvcBuilders.standaloneSetup(wedstrijdResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
